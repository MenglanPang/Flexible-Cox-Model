...
# Addition of weights in CoxFlex by Marie-Eve Beauchamp (MEB)
# Last update: August 6, 2019

# NOTE: Partial log-likelihood is not valid when weights are used.
#       A temporary solution to calculate AIC and LRTs is to normalize the weights to have mean of 1 
#       weights a each time during follow-up.
...
# Functions in this program:
  # backward_selection2 
  # CoxFlex
  # CoxFlex.weights  ##  Added by MEB to allow weights
  # DvlpMatrix
  # last_prog
  # last_prog.weights  ##  Added by MEB to allow weights (internal function)
  # lines.FlexSurv
  # plot.FlexSurv
  # spli
...
CoxFlex.weights <- function(data, Type, variables, TD, NL, m, p, knots, weights){
  
  res_principal<-last_prog.weights(data, Type, variables, TD, NL, m, p, knots, weights)
  ...
  for (kt in 1:length(variables)){
    if (TD[kt]==1){
      TDnew<-TD
      TDnew[kt]<-0
      resSEC<-last_prog.weights(data, Type, variables, TDnew, NL, m, p, knots, weights)
      TDtestP[kt]<-resSEC$Partial_Log_Likelihood
      TDtestN[kt]<-resSEC$Number_of_parameters
      TDtest[kt]<-1-pchisq(-2*(TDtestP[kt]-res_principal$Partial_Log_Likelihood),res_principal$Number_of_parameters-TDtestN[kt])
    }
    if (NL[kt]==1){
      NLnew<-NL
      NLnew[kt]<-0
      resSEC<-last_prog.weights(data, Type, variables, TD, NLnew, m, p,knots, weights)
      NLtestP[kt]<-resSEC$Partial_Log_Likelihood
      NLtestN[kt]<-resSEC$Number_of_parameters
      NLtest[kt]<-1-pchisq(-2*(NLtestP[kt]-res_principal$Partial_Log_Likelihood),res_principal$Number_of_parameters-NLtestN[kt])
    }
...
last_prog.weights <- function (data, Type, variables, TD, NL, m, p, knots=-999, weights) {
  ...
  variablesNEW<-match(variables,names(data)) 
  TypeNEW<-match(Type,names(data)) 
  weightsNEW <- match(weights,names(data)) ## Added by MEB: position of weights in data
  ...
  if (nbNLTD != 0) { 
    ...
    vrais <- c()
    modX <- coxph(eval(parse(text = substr(tt2, 13, nchar(tt2) - 
                                             1))), method = "efron", weights=QWR[,weightsNEW])
    vrais <- c(vrais, modX$loglik[2])
    ...
    modX<-coxph(eval(parse(text=substr(tt2,13,nchar(tt2)-1))),method="efron", weights=QWR[,weightsNEW])
    vrais<-c(vrais,modX$loglik[2])
